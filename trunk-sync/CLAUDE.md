...
- **git-block**: PreToolUse hook on Bash rejects any `git` command before execution, with feedback directing the agent to use Edit instead — `git clone`, `git diff`, `git log`, and `git show` are allowed through (read-only commands agents need for inspecting changes)
