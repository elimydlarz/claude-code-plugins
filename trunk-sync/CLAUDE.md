...
- **seance-read-only**: resumed agent is restricted to read-only tools (`--allowedTools`), forced into plan permission mode (`--permission-mode plan`) as a fallback, and given a system prompt (`--append-system-prompt`) enforcing seance mode — it cannot edit, write, or create files
