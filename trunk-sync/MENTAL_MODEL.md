...
- Agents register presence via committed, heartbeat-timestamped timecards, giving cross-machine visibility of who is active and what task their transcript describes.
...
- "Who is active" → `.trunk-sync/timeclock/<session-id>.json` (heartbeat `lastActiveAt`, branch, task), committed and pushed; surfaced at SessionStart.
...
- Timecard — `.trunk-sync/timeclock/<id>.json`; who is clocked in, where, when its heartbeat was last refreshed, and the task derived from its transcript.
- Liveness — the age of a card's heartbeat (`lastActiveAt`): active within the hour, stale (possibly disrupted) beyond it, reapable past a 14-day TTL. No PID and no clock-out command — the first edit creates the card.
...
- Timecards stay limited to liveness and task context; detailed handover belongs in tests, transcripts, or conversation, not an in-session progress protocol.
...
- At session start: the SessionStart hook hands the agent its session id and surfaces other sessions' timecards — active to coordinate around, stale (possibly disrupted) to verify against the tests before resuming.
