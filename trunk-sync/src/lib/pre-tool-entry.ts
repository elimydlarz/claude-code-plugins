...
function main(): void {
  ...
  try {
    ...
    else if (Array.isArray(value) && value.every((part) => typeof part === "string")) command = value.join(" ");
    else throw new Error("command must be a string or string array when provided.");
  } catch (error: unknown) {
