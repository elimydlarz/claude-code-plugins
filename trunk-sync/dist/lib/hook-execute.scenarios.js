...
import { mkdtempSync, writeFileSync, readFileSync, rmSync, mkdirSync, realpathSync, existsSync, } from "node:fs";
...
function makeState(dir, overrides = {}) {
...
function setupRepoWithRemote(prefix) {
...
describe("gatherRepoState", () => {
    ...
    it("detects repo root and git dir", () => {
        const origDir = process.cwd();
        process.chdir(dir);
        const state = gatherRepoState(makeInput({ tool_input: { file_path: join(dir, "file.txt") } }));
        process.chdir(origDir);
        assert.ok(state);
    ...
    it("detects file outside repo", () => {
        const origDir = process.cwd();
        process.chdir(dir);
        const state = gatherRepoState(makeInput({ tool_input: { file_path: "/tmp/outside.txt" } }));
        process.chdir(origDir);
        assert.ok(state);
    ...
    it("detects gitignored files", () => {
        ...
        const origDir = process.cwd();
        process.chdir(dir);
        const state = gatherRepoState(makeInput({ tool_input: { file_path: join(dir, "ignored.txt") } }));
        process.chdir(origDir);
        assert.ok(state);
    ...
    it("uses agents as targetBranch when a remote is configured", () => {
...
describe("executePlan", () => {
    ...
    it("enriches commit subject from transcript", () => {
        ...
        writeFileSync(transcriptPath, jsonl({ type: "user", message: { role: "user", content: "Fix the login bug" } }));
        const plan = {
            action: "commit-and-sync",
            commit: {
                filesToStage: [filePath],
    ...
    it("an enriched commit retains file, session, and agent provenance", () => {
        ...
        const transcriptPath = join(dir, "transcript.jsonl");
        writeFileSync(transcriptPath, jsonl({ type: "user", message: { role: "user", content: "Record provenance" } }));
        const sessionId = "abcdef12-3456-7890-abcd-ef1234567890";
        const plan = {
            action: "commit-and-sync",
            commit: {
...
describe("executeSync", () => {
    ...
    it("pulls and pushes to remote", () => {
        ...
        const sync = { targetBranch: "main", currentBranch: "main" };
    ...
    it("creates the target branch on first sync when it doesn't exist on the remote yet", () => {
        ...
        const sync = { targetBranch: "agents", currentBranch: "main" };
        const result = executeSync(sync);
        assert.equal(result.exitCode, 0);
    ...
    it("merges target branch on non-target worktree branch", () => {
        ...
        const sync = { targetBranch: "main", currentBranch: "trunk-sync-wt" };
        const result = executeSync(sync);
        assert.equal(result.exitCode, 0);
...
describe("clockIn", () => {
    ...
    it("preserves clockedInAt from existing timecard", () => {
        ...
        clockIn(dir, timecard);
        const content = JSON.parse(readFileSync(join(timeclockDir, "test-session.json"), "utf-8"));
...
describe("runStop", () => {
    ...
    function writeCard(sessionId, lastActiveAt) {
        const timeclockDir = join(dir, ".trunk-sync", "timeclock");
        mkdirSync(timeclockDir, { recursive: true });
...
describe("executePlan with timecard touch", () => {
    ...
    function writeOwnCard(sessionId = "my-session", lastActiveAt = new Date(Date.now() - 60_000).toISOString()) {
        const timeclockDir = join(dir, ".trunk-sync", "timeclock");
        mkdirSync(timeclockDir, { recursive: true });
        const cardPath = join(timeclockDir, `${sessionId}.json`);
        writeFileSync(cardPath, JSON.stringify({
    ...
    function commitAndSyncPlan(sessionId = "my-session") {
