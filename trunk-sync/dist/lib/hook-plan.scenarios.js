...
import { parseHookInput, planHook, buildCommitPlanWithTask, buildSessionPrefix, buildCommitBody, extractTaskFromTranscript, summarizeDeletions, classifyTimecards, formatClockInMessage, formatSessionStartSummary, } from "./hook-plan.js";
// ── Helpers ──────────────────────────────────────────────────────────
...
describe("planHook normal commit", () => {
    it("produces commit-and-sync for a file edit", () => {
        ...
        if (plan.action !== "commit-and-sync")
            return;
    ...
    it("Codex local_shell with no file_path stages dirty tracked files", () => {
        const input = makeInput({
            ...
            tool_input: { command: ["sed", "-i", "s/x/y/", "foo.ts"] },
        });
        const state = makeState({ modifiedFiles: ["foo.ts"], relPath: null });
        const plan = planHook(input, state);
        assert.equal(plan.action, "commit-and-sync");
...
describe("classifyTimecards", () => {
    ...
    function makeTimecard(overrides = {}) {
        return {
    ...
    it("classifies a card whose heartbeat is within the display window as active", () => {
        const result = classifyTimecards("my-session", [makeTimecard({ lastActiveAt: "2026-03-27T09:30:00.000Z" })], // 35 min ago
        now);
    ...
    it("classifies a card past the display window but within the reap ttl as stale", () => {
        const result = classifyTimecards("my-session", [makeTimecard({ lastActiveAt: "2026-03-27T08:00:00.000Z" })], // 2h5m ago
        now);
    ...
    it("classifies a card past the reap ttl as reapable", () => {
        const result = classifyTimecards("my-session", [makeTimecard({ lastActiveAt: "2026-03-10T10:00:00.000Z" })], // 17 days ago
        now);
