...
- In `codex exec` Default mode, `request_user_input` is unavailable. Functional scenarios that require a question should instruct Codex to ask in assistant text, and deterministic verifiers should fail if the transcript contains `request_user_input is unavailable`.
