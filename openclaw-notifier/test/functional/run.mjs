...
function run(env, stdin = '{}') {
  try {
    const result = execFileSync('bash', [NOTIFY_SH], {
      input: stdin,
...
console.log('  when OPENCLAW_URL is set and a subagent completes');
{
  ...
  const result = run({ OPENCLAW_URL: srv.url }, payload);
...
{
  const srv = await startServer();
  run({ OPENCLAW_URL: srv.url, OPENCLAW_TOKEN: 'secret-token' });
...
console.log('  when the server returns a non-2xx status');
{
  ...
  const result = run({ OPENCLAW_URL: srv.url });
...
{
  const result = run({ OPENCLAW_URL: 'http://127.0.0.1:1' });
