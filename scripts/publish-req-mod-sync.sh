...
if [[ ! "$BUMP" =~ ^(patch|minor|major)$ ]]; then
  echo "Usage: publish-req-mod-sync.sh <patch|minor|major>" >&2
  exit 1
fi
REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

cd "$REPO_ROOT/req-mod-sync"

# Source changes must be committed
DIRTY=$(git -C "$REPO_ROOT" status --porcelain -- 'req-mod-sync/')
if [ -n "$DIRTY" ]; then
...
git -C "$REPO_ROOT" add req-mod-sync/.claude-plugin/plugin.json
git -C "$REPO_ROOT" commit -m "req-mod-sync v$VERSION"
git -C "$REPO_ROOT" tag "req-mod-sync-v$VERSION"
...
echo ""
echo "published req-mod-sync v$VERSION"
