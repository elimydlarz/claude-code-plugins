...
**Never use a global install to verify a publish** (`npm install -g` / `pnpm add -g`), and never install the Claude Code *plugin* at `--scope user` — the plugin must stay project-scoped. The one legitimate global install is the CLI itself (`@elimydlarz/trunk-sync`, per the README's `npm install -g` onboarding step), and it is not this skill's job to create, update, or verify it — doing so ad hoc is exactly how the machine ended up with three dead identities (`trunk-sync@2.2.0` unscoped, `@susu-eng/trunk-sync` at several versions, and a stale `@elimydlarz/trunk-sync`) before a 2026-07-05 cleanup. To confirm a publish reached the registry, use `npm view @elimydlarz/trunk-sync version` (no local state) or `npx --yes @elimydlarz/trunk-sync@latest --version` (ephemeral, not installed). If the user's own global CLI install needs updating, that's `pnpm add -g @elimydlarz/trunk-sync@latest` — a deliberate, user-driven action, not a step this skill takes on its own.
