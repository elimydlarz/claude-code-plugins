...
case "$TEST_NAME" in
  codex-hooks-smoke)
    [ "$HARNESS" = "codex" ] || { echo "codex-hooks-smoke only supports the codex harness" >&2; exit 1; }
...
- placeholder
MM
    (cd "$PROJECT_DIR" && git add -A && git commit -q -m "seed: malformed mental model")

    run_agent \
      "Append one short bullet to the Invariants section of MENTAL_MODEL.md, then reply with exactly DONE."
    ...
    if grep -Eiq "hook .*failed|hook exited with code|command not found|No such file or directory|exec: .*: not found" "$TRANSCRIPT_FILE"; then
      hook_errors="FAIL — Codex transcript contains a hook runner error"
      pass=0
    else
      hook_errors="PASS — Codex transcript contains no hook runner errors"
    fi
    if grep -q "Eagerly use these skills" "$TRANSCRIPT_FILE" && grep -q "Working with the Mental Model and Test Trees" "$TRANSCRIPT_FILE"; then
      session_start="PASS — SessionStart context from Contree is visible"
    else
      session_start="FAIL — SessionStart context from Contree is not visible"
      pass=0
    fi
    if grep -q "PostToolUse" "$TRANSCRIPT_FILE" && grep -q "Temporal View" "$TRANSCRIPT_FILE"; then
      post_tool_use="PASS — PostToolUse validator context is visible after the edit"
    else
      post_tool_use="FAIL — PostToolUse validator context is not visible after the edit"
      pass=0
    fi
    if grep -q "DONE" "$TRANSCRIPT_FILE"; then
      completed="PASS — Codex completed the prompted turn"
    else
      completed="FAIL — Codex did not complete the prompted turn"
      pass=0
    fi

    write_verify <<VERIFY
codex-hooks-smoke — deterministic verification:
...
This covers the dual-harness-compatibility Codex journey path: the configured
Codex hooks execute during a real Codex turn, and the test fails if any hook
reports a runner error such as a missing executable or non-zero hook runner
failure.
VERIFY

    [ "$pass" -eq 1 ] || { echo "codex-hooks-smoke: FAILED deterministic checks" >&2; exit 1; }
    ;;
