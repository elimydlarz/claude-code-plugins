...
- **`/contree:setup`** — Configures your test framework with tree reporters, normal and functional test commands, a conventional linter with strong rules, save-time autofix and architecture enforcement through project-level hooks, and creates `TEST_TREES.md` when needed. Run once per project.
...
1. Run `/contree:setup` — sets up the test framework, normal and functional test commands, strong linting with project-level autofix and architecture hooks, and creates `TEST_TREES.md` when needed
