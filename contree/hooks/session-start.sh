...
- **Outside-in TDD** — start from the behaviour in the current tree and its consumer. Write a test, observe RED, implement GREEN, then during REFACTOR observe branching under different conditions. Mock a unit that can encapsulate that observed branching, simplify the consumer tests so they require the mock to be consumed correctly, change the consumer to consume it, and TDD that unit recursively. Do not design lower units before passing consumer tests reveal their need.
