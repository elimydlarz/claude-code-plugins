...
if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ]; then
  LAST_CHAR=$(jq -rs '
    ([.[] | select(.type == "assistant") | .message.content[]? | select(.type == "text") | .text] | first // "")
    | sub("[[:space:]]+$"; "")
...
echo 'Guard the contract. (1) Has implementation drifted from test trees in ## Requirements? Test trees are the contract between intent and implementation — never modify them silently. If drift exists, ask the user: update test trees and tests to reflect the implementation, or pare implementation back to match the test trees? (2) Has the public API or behaviour changed in a way not reflected in ## Mental Model? Any new parameter, changed signature, added capability, or removed feature means the Mental Model needs updating. (3) Does CLAUDE.md need updating? (4) Does README.md accurately describe the current state of the project? If nothing needs attention, reply only with 0.' >&2
exit 2
