...
- **change-without-me skill** — the full arc. Runs `change` → `sync` → `tdd` → mutation → completion sync → `second-opinion`, repairing review findings through the same gates — from idea to verified, independently reviewed working software.
...
- `test/dual-harness-compatibility.bats` — dual-harness contract: both manifests, version lockstep, plugin.json fields, shared hooks.json, `$CLAUDE_PLUGIN_ROOT` invocation, Stop hook wiring, OpenAI Responses authentication, gpt-5.6-luna at medium reasoning effort, a $5 standard Claude journey budget ceiling, and fail-fast missing-key handling
...
- `test/fixtures/bookmarks-api/` — HTTP API fixture for the outside-in workflow across Journey, System, Component, Adapter, Use-case, Domain, and Port tests
